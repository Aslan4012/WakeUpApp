...
class RingScreen extends StatelessWidget {
  ...
  const RingScreen(
      {Key? key, required this.alarmSettings, required this.challenge})
      : super(key: key);
    ...
    switch (challenge) {
      case 'Math':
        return MaterialPageRoute(
            builder: (context) => MathChallenges(alarmId: alarmSettings.id));
      case 'Walk':
        return MaterialPageRoute(
            builder: (context) => WalkScreen(alarmId: alarmSettings.id));
      case 'Spin':
        return MaterialPageRoute(
            builder: (context) => SpinScreen(alarmId: alarmSettings.id));
      case 'Shake':
        return MaterialPageRoute(
            builder: (context) => ShakeScreen(alarmId: alarmSettings.id));
      default:
        throw Exception(
            'SOMEHOW PASSED ON: $challenge'); //WONT HAPPEN, PLEASE....
    }
  }
  ...
  void _handleSnooze(BuildContext context) {
    ...
    Alarm.set(
      alarmSettings: alarmSettings.copyWith(
        dateTime: DateTime(snoozeTime.year, snoozeTime.month, snoozeTime.day,
            snoozeTime.hour, snoozeTime.minute, 0, 0),
      ),
    ).then((_) {
